<template>
	<div class="movie_body">
		<div class="wrapper">
			<ul class="movie-list">
				<router-link tag='li' :to='{name : "Page",query:{id : item.id}}' class="movie-list-item" v-for='item in movieArr' :key='item.nm'>
					<div class="movie-img">
						<img :src="getImg(item.img)" alt="">
					</div>
					<div class="movie-content">
						<h2 class="movie-title">{{item.nm}}
							<img v-if='item.version' :src="max3d" alt="">
						</h2>
						<p>
							观众评 <span class="grade">{{item.sc}}</span>
						</p>
						<p class="movie-info">主演: {{item.star}}</p>
						<p>{{item.showInfo}}</p>
					</div>
					<div class="btn_buy">购票</div>
				</router-link>
			</ul>
		</div>
	</div>
</template>
<script>
	export default{
		name : 'movie',
		data(){
			return {
				movieArr : [],
				max3d : 'data:image/png;base64,',
				id : 296
			}
		},
		created(){
			var storage = localStorage.getItem('curCity');
		    if(storage){
		       var obj = JSON.parse(storage);
		       this.id = obj.id;
		    }
		},
		mounted(){
			this.axios.get('/api/movieOnInfoList?cityId='+this.id).then((val)=>{
				if(val.status === 200){
					this.movieArr = val.data.data.movieList;
				}
			})
		},
		methods : {
			getImg(img){
				return 'http://p1.meituan.net/128.180/movie' + img.slice(img.lastIndexOf('/'));
			}
		}
	}
</script>
<style scoped>
	.movie_body{
		flex:1;
		overflow: auto;
	}
	.movie_body .wrapper{
		height: 100%;
	}
	.movie_body .wrapper .movie-list{
		margin:0 12px;
		overflow: hidden;
	}
	.movie_body .wrapper .movie-list .movie-list-item{
		margin-top:12px;
		display: flex;
		padding-bottom: 10px;
		border-bottom: 1px solid #e6e6e6;
		align-items:center;
	}
	.wrapper .movie-list .movie-list-item .movie-img{
		width:64px;
		height: 90px;
	}
	.movie-list .movie-list-item .movie-img img{
		width:100%;
		height: 100%;
	}
	.wrapper .movie-list .movie-list-item .movie-content{
		flex:1;
		margin-left: 10px;
		position: relative;
		text-align: left;
	}
	.movie-list .movie-list-item .movie-content .movie-title{
		font-size: 17px;
		width:150px;
		line-height: 24px;
	}
	.movie-list-item .movie-content .movie-title img{
		position: absolute;
		width: 50px;
		top:5px;
		right: 10px;
	}
	.movie-list .movie-list-item .movie-content p{
		font-size: 14px;
		color:#666;
		width:200px;
		line-height: 22px;
	}
	.movie-list .movie-list-item .movie-content h2,
	.movie-list .movie-list-item .movie-content p{
		overflow: hidden;
		white-space: nowrap;
		text-overflow: ellipsis;
	}
	.movie-list .movie-list-item .btn_buy{
		width:47px;
		height: 27px;
		line-height: 28px;
		border-radius: 4px;
		font-size: 12px;
		color:#fff;
		cursor: pointer;
		background-color: #f03d37;
		text-align:center;
	}
</style>